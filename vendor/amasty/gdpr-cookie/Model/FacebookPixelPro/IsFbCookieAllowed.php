...
namespace Amasty\GdprCookie\Model\FacebookPixelPro;
...
class IsFbCookieAllowed
{
    public const COOKIE_FACEBOOK_PIXEL = '_fbp';
     ...
     */
    private CookieManagementInterface $cookieManagement;
    ...
    public function execute(): bool
    {
        ...
        foreach ($this->cookieManagement->getEssentialCookies($storeId) as $essentialCookie) {
            if ($essentialCookie->getName() === self::COOKIE_FACEBOOK_PIXEL) {
                $isFbEssential = true;
            }
        }
        ...
        if ((!$allowedGroups || $allowedGroups === CookieManager::ALLOWED_NONE) && !$isFbEssential) {
            $isFbAllowed = false;
        }
        ...
        if ($allowedGroups) {
            ...
            foreach ($rejectedCookies as $cookie) {
                if ($cookie->getName() === self::COOKIE_FACEBOOK_PIXEL) {
                    $isFbAllowed = false;
                    break;
                }
            }
