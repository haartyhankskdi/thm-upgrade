...
use Ebizmarts\SagePaySuite\Model\PiRequestManagement\TransactionAmountFactory;
use Ebizmarts\SagePaySuite\Model\PiRequestManagement\TransactionAmountPost;
use Ebizmarts\SagePaySuite\Model\PiRequestManagement\TransactionAmountPostFactory;
...
{
    private const DELIVERY_PHONE_LENGTH_FORM_AND_REPEAT = 19;
    private const DELIVERY_PHONE_LENGTH_PAYPAL_AND_SERVER = 20;
    ...
    /** @var TransactionAmountFactory */
    private $transactionAmountFactory;

    /** @var TransactionAmountPostFactory */
     ...
     * @param TransactionAmountFactory $transactionAmountFactory
     * @param TransactionAmountPostFactory $transactionAmountPostFactory
     * @param PriceCurrencyInterface $priceCurrency
     * @param \Ebizmarts\SagePaySuite\Helper\ValidateRequest $validateRequest
     */
    ...
    public function populateAddressInformation($quote)
    {
        ...
        $data['BillingAddress1']   = substr($billing_address->getStreetLine(1), 0, 50);
        $data['BillingAddress2']   = substr($billing_address->getStreetLine(2), 0, 50);
        $data['BillingCity']       = substr($billing_address->getCity(), 0, 40);
        $data['BillingPostCode']   = !empty($billing_address->getPostcode())
            ? substr($billing_address->getPostcode(), 0, 10)
            : '000';
        ...
        $data['BillingPhone'] = !empty($billing_address->getTelephone())
            ? substr($billing_address->getTelephone(), 0, 19)
            : '';
        ...
        $data['DeliveryAddress1']   = substr($shipping_address->getStreetLine(1), 0, 50);
        $data['DeliveryAddress2']   = substr($shipping_address->getStreetLine(2), 0, 50);
        $data['DeliveryCity']       = substr($shipping_address->getCity(), 0, 40);
        $data['DeliveryPostCode']   = !empty($shipping_address->getPostcode())
            ? substr($shipping_address->getPostcode(), 0, 10)
            : '000';
        ...
        $data['DeliveryPhone'] = !empty($shipping_address->getTelephone())
            ? substr(
                $shipping_address->getTelephone(),
                0,
                $this->getDeliveryPhoneLength($quote->getPayment()->getMethod())
            ) : '';
    ...
    public function populatePaymentAmountAndCurrency($quote, $isRestRequest = false)
    {
        ...
        if ($isRestRequest) {
            ...
            /** @var TransactionAmountPost  $transactionAmount */
    ...
    private function getBasketXml($quote)
    {
        ...
        //If all products are virtual there is no shipping address
        if ($this->isAddressEmpty($shippingAdd)) {
        ...
        foreach ($itemsCollection as $item) {
            ...
            //<recipientAdd1>
            $address1 = $this->validateRequest->stringToSafeXMLChar(
                substr(trim($shippingAdd->getStreetLine(1)), 0, 100),
                ValidateRequest::SHIPPING_STREET
            );
            if (!empty($address1)) {
            ...
            if ($shippingAdd->getStreet(2)) {
                $recipientAdd2 = $this->validateRequest->stringToSafeXMLChar(
                    substr(trim($shippingAdd->getStreetLine(2)), 0, 100),
                    ValidateRequest::SHIPPING_STREET
                );
            ...
            //<recipientCity>
            $recipientCity = $this->validateRequest->stringToSafeXMLChar(
                substr(trim($shippingAdd->getCity()), 0, 40),
                ValidateRequest::SHIPPING_CITY
            );
            ...
            //<recipientCountry>
            $node->addChild('recipientCountry', $this->validateRequest->stringToSafeXMLChar(
            ...
            $node->addChild('recipientPostCode', $this->validateRequest->stringToSafeXMLChar(
                $this->sanitizePostcode(substr(trim($_postCode), 0, 9)),
                ValidateRequest::SHIPPING_POSTCODE
            ));
        }
    ...
    private function basketXmlRecipientFName($shippingAdd, $node)
    {
        ...
        if ($validFName > 0) {
            $this->addChildCData(
                'recipientFName',
                substr(
                    $this->sanitizeRecipientName($this->validateRequest->stringToSafeXMLChar(
                        $shippingAdd->getFirstname(),
                        ValidateRequest::SHIPPING_NAME
                    )),
                    0,
                    20
                ),
    ...
    private function basketXmlMiddleName($shippingAdd, $node)
    {
        if ($shippingAdd->getMiddlename()) {
            $recipientMName = $this->validateRequest->stringToSafeXMLChar(
                substr(trim($shippingAdd->getMiddlename()), 0, 1),
                ValidateRequest::SHIPPING_MIDDLE_NAME
            );
    ...
    private function basketXmlProductSku($item, $node)
    {
        ...
        if ($validSku === 1) {
            ...
            $this->addChildCData(
                'productSku',
                substr(
                    $this->sanitizePostcode($this->validateRequest->stringToSafeXMLChar(
                        $item->getSku(),
                        ValidateRequest::SKU
                    )),
                    0,
    ...
    private function basketXmlProductDescription($item, $node)
    {
        ...
        $this->addChildCData('description', $this->validateRequest->stringToSafeXMLChar(
            $itemDesc,
            ValidateRequest::ITEM_DESCRIPTION
        ), $node);
    }
    ...
    private function basketXmlRecipientSalutation($shippingAdd, $node)
    {
        if ($shippingAdd->getPrefix()) {
            $recipientSal = $this->validateRequest->stringToSafeXMLChar(
                substr(trim($shippingAdd->getPrefix()), 0, 4),
                ValidateRequest::SHIPPING_PREFIX
            );
    ...
    private function basketXmlRecipientEmail($shippingAdd, $node)
    {
        if ($shippingAdd->getEmail()) {
            $recipientEmail = $this->validateRequest->stringToSafeXMLChar(
                substr(trim($shippingAdd->getEmail()), 0, 45)
            );
    ...
    {
        $recipientPhone = $this->validateRequest->stringToSafeXMLChar(
            substr(trim($shippingAdd->getTelephone()), 0, 20),
            ValidateRequest::SHIPPING_PHONE
        );
    ...
    private function basketXmlRecipientState($quote, $shippingAdd, $node, $billingAdd)
    {
        if ($shippingAdd->getCountry() == 'US') {
            ...
            $node->addChild('recipientState', $this->validateRequest->stringToSafeXMLChar(
                $regionCode
            ));
        }
    }
     ...
     * @return int
     */
    private function getDeliveryPhoneLength($methodCode)
    {
        return $this->isFormOrRepeatMethodCode($methodCode)
            ? self::DELIVERY_PHONE_LENGTH_FORM_AND_REPEAT
            : self::DELIVERY_PHONE_LENGTH_PAYPAL_AND_SERVER;
    }
     ...
     * @return bool
     */
    private function isFormOrRepeatMethodCode($methodCode)
    {
        return $methodCode === Config::METHOD_FORM || $methodCode === Config::METHOD_REPEAT;
    }

    private function isAddressEmpty($address)
    {
        $requiredFields = ['firstname', 'lastname', 'street', 'city', 'country_id'];
        $isEmpty = false;
        foreach ($requiredFields as $field) {
            if (empty($address->getData($field))) {
                $isEmpty = true;
                break;
            }
        }
