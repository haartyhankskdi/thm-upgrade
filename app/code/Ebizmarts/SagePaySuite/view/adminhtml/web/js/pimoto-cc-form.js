...
define([
    ...
    "sagepayerror",
    'mage/translate'
], function ($, url, ui, $validation, sagepayerror) {
    "use strict";
    ...
    if (typeof order !== 'undefined') {
        order.addExcludedPaymentMethod('sagepaysuitepimoto');
    }

    $.widget('mage.sagepaysuitepimotoCcForm', {
        options: {
            code: "sagepaysuitepimoto"
        },
        creditCardType: '',
        creditCardExpYear: '',
        creditCardExpMonth: '',
        ...
        _init: function () {
            ...
            $('#sagepaysuite-info-message' + randomNumber).hide();
            if(this.dropInEnabled()) {
                $("#submit_dropin_payment").click(function () {
                    window.order.submit();
                });
            }
        },
                ...
                require([jsName + this.options.mode], function () {
                    $.ajax({
                        ...
                        if (response.success) {
                            ...
                            sagepayerror.showPaymentError(self.getCode(), $.mage.__(response.error_message ? response.error_message : response.message));
                        }
                    });
                });
            }
        ...
        sagepayTokeniseCard: function (merchant_session_key) {
            ...
            if (self.dropInEnabled()) {
                if (merchant_session_key) {
                    ...
                    if (typeof window.dropInInstance !== 'undefined' && window.dropInInstance !== null) {
                            ...
                            onTokenise: function (tokenisationResult) {
                                if (tokenisationResult.success) {
                                    ...
                                    try {
                                        ...
                                        console.log(err);
                                        sagepayerror.showPaymentError(self.getCode(), $.mage.__("Unable to initialize Opayo payment method, please use another payment method."));
                                    }
                                } else {
                                    ...
                                    if (tokenisationResult.error.errorCode == 1002) {
                                        ...
                                        sagepayerror.showPaymentError(self.getCode(), $.mage.__('Tokenisation failed', tokenisationResult.error.errorMessage));
                                    }
                                }
                            }
                        });
                    ...
                    window.dropInInstance = self.dropInInstance;
                }
            } else {
                if (merchant_session_key) {
                    ...
                    try {
                        ...
                        Sagepay.tokeniseCardDetails(token_form, function (status, response) {
                            if (status === 201) {
                                ...
                                sagepayerror.showPaymentError(self.getCode(), $.mage.__(self.processNokResponse(status, response)));
                            }
                        });
                    } catch (err) {
        ...
        placeTansactionRequest: function () {
            ...
            $.ajax({
                ...
                if (response.success == true) {
                    ...
                    sagepayerror.showPaymentError(self.getCode(), $.mage.__(response.error_message ? response.error_message : "Invalid Opayo response, please use another payment method."));
                    if (self.dropInEnabled()){
                        self.dropInInstance.destroy();
                        $('#dropin_start_lbl').show();
                        document.getElementById('submit_dropin_payment').style.display = "none";
    ...
    return $.mage.sagepaysuitepimotoCcForm;
});
