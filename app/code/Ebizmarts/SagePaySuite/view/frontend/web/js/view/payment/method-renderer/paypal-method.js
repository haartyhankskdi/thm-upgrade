...
define(
    ...
    function ($, Component, storage, url, urlBuilder, customer, quote, fullScreenLoader, additionalValidators, customerData, setPaymentInformation) {
        'use strict';
        var template = 'Ebizmarts_SagePaySuite/payment/paypal-form';
        var paypalConfig = window.checkoutConfig.payment.ebizmarts_sagepaysuitepaypal;
        if (paypalConfig && !paypalConfig.licensed && paypalConfig.mode == "live") {
        ...
        return Component.extend({
                            ...
                            function () {
                                ...
                                if (customer.isLoggedIn()) {
                                    paypalServiceUrl = urlBuilder.createUrl(
                                        '/elavon/paypal/:cartId',
                                        {cartId: quote.getQuoteId()}
                                    );
                                } else {
                                    paypalServiceUrl = urlBuilder.createUrl(
                                        '/elavon/paypal-guest/:cartId',
                                        {cartId: quote.getQuoteId()}
                                    );
                                }
                                var browserData = {
            ...
            isLicenced: function () {
                return window.checkoutConfig.payment.ebizmarts_sagepaysuitepaypal.licensed;
            }
        });
    }
);
