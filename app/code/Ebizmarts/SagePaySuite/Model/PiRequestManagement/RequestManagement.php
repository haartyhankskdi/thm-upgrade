...
abstract class RequestManagement implements \Ebizmarts\SagePaySuite\Api\PiOrderPlaceInterface
{
    ...
    {
        $this->payResult = $this->getPiRestApi()->capture($this->getRequest($isMoto));
    ...
    public function processPayment($isMoto = false)
    {
        if ($this->isSuccessOrThreedAuth()) {
            $this->saveAdditionalPaymentInformation($isMoto);
            ...
            throw new ValidatorException(
                __('Invalid Elavon response. %1', $statusDetail)
            );
        }
    }

    private function saveAdditionalPaymentInformation($isMoto)
    {
        $paymentMethod = $isMoto ? Config::METHOD_PI_MOTO : Config::METHOD_PI;
        $payResult = $this->getPayResult();
        $this->getPayment()->setMethod($paymentMethod);
        ...
        $this->getPayment()->setAdditionalInformation('moto', $isMoto);
