...
class Form extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    public function decodeSagePayResponse($crypt)
    {
        ...
        if (false !== $strDecoded) {
            $responseRaw = $this->getRawResponse($strDecoded);
            ...
            for ($i = 0; $i < $responseRawCnt; $i++) {
                ...
                try {
                    ...
                    $this->sagePayLogger->logException($exception, [__METHOD__, __LINE__]);
                    $this->sagePayLogger->sageLog(SagePayLogger::LOG_EXCEPTION, $responseRaw, [__METHOD__, __LINE__]);
                    continue;
                }
            }
        }
    ...
    {
        return \Ebizmarts\SagePaySuite\Block\Info::class;
    }
    ...
    /**
     * @param string $strDecoded
     * @return string[]
     */
    private function getRawResponse(string $strDecoded)
    {
        $initPosition = strpos($strDecoded, "&SchemeTraceID=");
        if ($initPosition === false) {
            $responseRaw = explode('&', $strDecoded);
        } else {
            $endPosition = strpos($strDecoded, "&Amount=");
            $lengthToRemove = $endPosition - $initPosition;
            $schemeTraceIdString = substr(substr($strDecoded, $initPosition, $lengthToRemove), 1);
            $strDecoded = substr_replace($strDecoded, '', $initPosition, $lengthToRemove);
            $responseRaw = explode('&', $strDecoded);
            $responseRaw[] = $schemeTraceIdString;
        }
