...
class OrderUpdateOnCallback
{
    ...
    {
        if ($this->order === null) {
            throw new LocalizedException(__("Invalid order. Cant confirm payment."));
        }
    ...
    public function sendInvoiceNotification()
    {
        if ($this->invoiceConfirmationIsEnable() && $this->paymentActionIsCapture()) {
            ...
            if ($invoices->count() > 0) {
                ...
                $this->suiteLogger->debugLog('Invoice confirmation sent for order: ' .
                $this->order->getIncrementId(), [__METHOD__, __LINE__]);
            }
        }
    }
