...
class PiTransactionResultCard extends AbstractExtensibleModel implements PiTransactionResultCardInterface
{
    ...
    {
        return $this->getData(self::CARD_IDENTIFIER);
    }
    ...
    {
        return $this->getData(self::REUSABLE);
    }
    ...
    {
        return [
            self::CARD_TYPE => $this->getCardType(),
            self::LAST_FOUR_DIGITS => $this->getLastFourDigits(),
            self::EXPIRY_DATE => $this->getExpiryDate(),
            self::CARD_IDENTIFIER => $this->getCardIdentifier(),
            self::REUSABLE => $this->getIsReusable()
        ];
    }
}
