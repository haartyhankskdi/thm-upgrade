...
use Ebizmarts\SagePaySuite\Model\InvalidTransactionIdException;
...
class Notify implements ActionInterface, CsrfAwareActionInterface
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if (!empty($transactionId) && $payment->getLastTransId() == $transactionId) { //validate transaction id
                ...
                $expiryDateMonth = $this->postData->ExpiryDate !== null
                    ? substr($this->postData->ExpiryDate, 0, 2)
                    : null;
                $expiryDateYear = $this->postData->ExpiryDate !== null
                    ? substr($this->postData->ExpiryDate, 2)
                    : null;
                $payment->setCcExpMonth($expiryDateMonth);
                $payment->setCcExpYear($expiryDateYear);
                $payment->save();
            } else {
                throw new InvalidTransactionIdException(__('Invalid transaction id'));
            }
            ...
            if ($status == "ABORT") { //Transaction canceled by customer
                ...
                if ($state === Order::STATE_PENDING_PAYMENT) {
                    /*The order might be cancelled on Model/recoverCart
                    if SagePay takes too long in sending the notify.
                    This checks if the order is not cancelled before trying to cancel it.*/
    ...
    private function getVPSSignatureString($payment)
    {
        ...
        (property_exists($this->postData, 'ACSTransID') === true ? $this->postData->ACSTransID : '') .
        (property_exists($this->postData, 'DSTransID') === true ? $this->postData->DSTransID : '') .
        (property_exists($this->postData, 'SchemeTraceID') === true ? $this->postData->SchemeTraceID : '');
    }
    ...
    private function returnInvalid(
        $message = 'Invalid transaction, please try another payment method',
        $quoteId = null,
        $orderId = null
    ) {
    ...
    private function getAbortRedirectUrl($quoteId = null, $orderId = null)
    {
        ...
        $url .= "&quote=" . urlencode($quoteId) . "&message=".__('Transaction cancelled by customer');
    ...
    {
        $url = $this->url->getUrl('*/*/redirectToSuccess', [
            '_nosid' => true,
            '_secure' => true,
