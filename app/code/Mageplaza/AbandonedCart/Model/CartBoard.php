...
class CartBoard
{
    ...
    public function getData()
    {
        ...
        try {
            $data = [
                'realtime'    => $this->getCartData($this->getQuotesIsActive($timeMeasure, '>=')),
                'abandoned'   => $this->getCartData($this->getQuotesIsActive($timeMeasure, '<=', false)),
                'recoverable' => $this->getCartData($this->getQuotesIsActive($timeMeasure, '<=', true)),
                'converted'   => $this->getCartData($this->filterDateRange(0))
            ];
        } catch (Exception $e) {
